...
public class LabelSet{
    ...
    public static class LabelSetMapper extends Mapper<Object, Text, Text, Text>{
        ...
        protected void map(Object key, Text value, Context context)
            ...
            Map<String, Integer> name_label = new HashMap<String, Integer>();
            //System.out.println(unames);
            /*for(int i = 0; i < unames.length; i++){
                //System.out.println(unames[i]);
                int order = name_label.get(unames[i].split(",")[0]);
            ...
            for(String uname : unames){
                if(name_label.get(uname) == null){
                    name_label.put(uname, 1);
                }else{
                    int count = name_label.get(uname);
                    name_label.remove(uname);
                    name_label.put(uname, count + 1);
                }
            }
            int sum = 0;
            String label = null;
            for(String i : name_label.keySet()){
                if(name_label.get(i) >= sum){
                    label = i;
                    sum = name_label.get(i);
                }
            }
            String newvalue = label + "#" + label_list.split("#")[1];
